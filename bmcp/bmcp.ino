...
void PrintBoard()
{
    ...
    {
        if(!(sq % 16)) Serial.printf(" %d  ", 8 - (sq / 16));    // print ranks to the left of the board
        Serial.printf(" %c", ((sq & 8) && (sq += 7)) ? '\n' : pieces[board[sq] & 15]);    // ASCII pieces
        //Serial.printf(" %s", ((sq & 8) && (sq += 7)) ? "\n" : pieces[board[sq] & 15]);    // unicode pieces
    }
...
int main()
{
    ...
    int depth = 5;    // search depth (2 - 5 recommended)
...
#define DEBUG    // uncoment to see computer playing itself
    ...
    while(1)    // computer plays against itself
    {
        ...
        PrintBoard();
        Serial.printf("\nscore: '%d'\n", score);
        Serial.printf("best move: '%s%s'\n", notation[best_src], notation[best_dst]);
        
        while(!Serial.available());//getchar();
        Serial.read();
    }
    ...
    while(1)  // play vs computer
    {
        ...
        // Checkmate detection
        if(score == 10000 || score == -10000) {Serial.printf("Checkmate!\n"); break;}
