...
#define U (1<<20)
EXTMEM struct _ {int K,V;char X,Y,D;} A[U];           /* hash table, 16M+8 entries*/
...
int D(int q, int l, int e, int E, int z, int n)          /* recursive minimax search, k=moving side, n=depth*/
    ...
    {A:                                        /* resume normal after best */
     ...
     do{                                       /* y traverses ray, or:     */
      ...
      {v=p<6?b[x+8]-b[y+8]:0;                  /* center positional pts.   */
       ...
       //Serial.write(N&8&&(N+=7)?10:".?+nkbrq?*?NKBRQ"[b[N]&15]);
...
if(z)Serial.printf("%2d ply, %9d searched, score=%6d by %c%c%c%c\n",d-1,N-S,m,
...
void loop()
{
 ...
 W(1)                                                /* play loop          */
 {N=-1;W(++N<121)
   Serial.write(N&8&&(N+=7)?10:".?+nkbrq?*?NKBRQ"[b[N]&15]); /* print board */
  W(!Serial.available());
  p=c;W((*p++=Serial.read())>10);                        /* read input line    */
  K=I;     /* invalid move       */
  Serial.println("thinking");
  if(*c-10)K=*c-16*c[1]+799,L=c[2]-16*c[3]+799;      /* parse entered move */
  D(-I,I,Q,O,1,2);                                   /* think or check & do*/
 }
}
